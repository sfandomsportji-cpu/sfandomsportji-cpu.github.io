...
const startSignatureMoments=()=>{
  ...
  const MOMENT_MS=12000,END_MS=3200,VIDEO_LOAD_TIMEOUT_MS=12000;
  const moments=[
    {a:'OHTANI',b:'50–50',sub:"MLB's first 50-HR / 50-SB season · 2024",start:0,ms:30000,gamePk:746011,exact:'shohei ohtani homers, creates the 50-50 club'},
  ...
  const cache=new Map();
  let current=0,timer=null,endTimer=null,loadTimer=null,loadToken=0;
  ...
  async function render(){
    clearTimeout(timer);clearTimeout(endTimer);clearTimeout(loadTimer);
    end.classList.remove('show');
    const token=++loadToken;
    setMeta(current);
    clearPlayer();
    try{
      if(!player.canPlayType('video/mp4'))throw new Error('mp4 unsupported');
      const v=await resolveMoment(current);
      if(token!==loadToken)return;
      player.src=v.url;
      player.muted=true;
      player.playsInline=true;
      loadTimer=setTimeout(()=>{if(token===loadToken)next()},VIDEO_LOAD_TIMEOUT_MS);
      const startPlayback=()=>{
        if(token!==loadToken)return;
        clearTimeout(loadTimer);
      ...
      player.onseeked=()=>{clearTimeout(loadTimer);player.play().catch(()=>{});scheduleNext();resolveMoment((current+1)%moments.length).catch(()=>{})};
      player.onended=next;
      player.onerror=next;
    }catch(e){next()}
  }
...
const loadSecondary=()=>{
  import('./visitor-counter-bootstrap.js?v=20260917-counterrepair2').catch(()=>{});
