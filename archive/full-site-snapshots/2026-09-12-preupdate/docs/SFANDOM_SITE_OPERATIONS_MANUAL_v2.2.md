...
# 21. ERROR STOP & NORMALIZATION — 오류 1건 발생 시 즉시 정상화 시스템

## 21.1 발동 조건

작업 중 오류가 **1건이라도 발생하면 즉시 현재 작업을 중지한다.**

오류의 예:
- GitHub API/PR/Merge/Draft/Permission 오류
- 이미지 로드 실패, 잘못된 선수 이미지, 경로 오류
- HTML/CSS/JS Console Error
- 예상하지 않은 파일 변경
- 링크/데이터/시간/선발 검증 실패
- 배포 결과가 예상과 다름
- MODULE 응답 실패
...
오류의 크기나 SEV 등급을 판단하기 전에 먼저 중지한다.

## 21.2 즉시 행동

오류 감지 즉시 다음 순서로 진행한다.

1. **STOP** — 추가 Commit, Merge, 배포, 파일 수정, 임시 Hotfix를 중단한다.
2. **FREEZE** — 현재 정상 상태와 작업 Branch/Commit/PR 상태를 그대로 보존한다.
3. **RECORD** — 오류 메시지, 발생 시각(KST), 직전 정상 단계, 변경 파일을 기록한다.
4. **ISOLATE** — 오류가 TOOL / CONTENT / MODULE / CORE / DEPLOY 중 어디에서 발생했는지 분리한다.
5. **COMPARE** — 직전 정상 Commit/Branch와 현재 상태의 Diff를 확인한다.
6. **NORMALIZE** — 오류 원인을 제거하거나 정상 경로로 복구한다.
7. **VERIFY** — 동일 오류 재현 여부, 변경 파일, PC 1440px, Mobile 390px, 이미지/링크, 관련 기능을 재검증한다.
8. **RESUME** — 정상화가 확인된 뒤에만 중단된 지점부터 작업을 재개한다.

## 21.3 금지 행동

오류 발생 후 다음 행동을 금지한다.

- “코드는 안 꼬일 것 같다”는 추정만으로 계속 진행
- 오류를 무시하고 Merge/배포 강행
- 원인 미확인 상태에서 다른 코드까지 수정
- 오류를 덮기 위한 임시 CSS/JS 추가
- 새로운 Branch/PR을 여러 개 만들어 문제를 분산
- 정상화 확인 전에 사용자 승인 내용을 임의 변경
- 도구 오류를 사이트 코드 오류로 단정
- 사이트 코드 오류를 도구 오류로 단정

## 21.4 정상화 완료 조건

다음 조건을 모두 충족해야 작업을 재개할 수 있다.

- 최초 오류 원인 확인 또는 안전한 우회 경로 확인
- 동일 오류가 다시 발생하지 않음
- 직전 정상 상태 대비 예상 외 Diff 없음
- 관련 기능 정상
- 이미지/링크 정상
- PC 1440px PASS
- Mobile 390px PASS
...
하나라도 충족하지 못하면 **계속 STOP 상태**를 유지한다.

## 21.5 승인과 오류의 관계

사용자 승인이 이미 있더라도 오류가 1건 발생하면 정상화 시스템이 우선 발동한다.

단, 정상화 완료 후에는:
- 기존 승인 범위가 변경되지 않았으면 **같은 승인으로 작업을 재개**한다.
- 콘텐츠/코드/대상 파일/배포 범위가 바뀌었다면 **재승인**을 받는다.

즉, 오류가 발생했다고 승인 자체를 취소하거나 임의로 보류하는 것이 아니라, **작업만 일시 정지하고 정상화 후 승인 범위 안에서 재개**한다.

## 21.6 핵심 운영 문장

**오류 1건 = 즉시 STOP.**  
**STOP은 취소가 아니라 정상화 시작이다.**  
**정상화 확인 전에는 한 줄도 더 밀지 않는다.**  
**정상화 완료 후에는 기존 승인 범위 안에서 즉시 재개한다.**
...
# 25. 이번 대화에서 확정한 운영 결정 기록 — 2026-08-31
...
11. 작업 중 오류가 1건이라도 발생하면 즉시 STOP하고 정상화 시스템을 가동한다. 정상화 확인 후 기존 승인 범위 안에서 작업을 재개한다.
